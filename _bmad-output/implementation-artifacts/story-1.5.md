...
- [ ] Task 1: Create template resolver (AC: #1, #2, #4)
  - [ ] Create `src/templates/resolver.js` with `resolveTemplate(templateName)` function
  - [ ] Implement user-first resolution chain: check `src/templates/user/{name}` then `src/templates/default/{name}`
  - [ ] Implement error for missing templates
  - [ ] Create `test/templates/resolver.test.js` covering all 3 paths (user override, default fallback, missing)
- [ ] Task 2: Integrate backup into install logic (AC: #3)
  - [ ] Update `src/install.js` (from Story 1.2) to use resolver for collision detection
  - [ ] Implement `.bak` creation before overwriting default templates
